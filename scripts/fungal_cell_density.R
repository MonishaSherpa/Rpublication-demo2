...
group_summary_num <- df_num %>%
  group_by(sample) %>%
  summarise(mean_density = mean(fungal_cell_density, na.rm = TRUE),
            sd_density = sd(fungal_cell_density, na.rm = TRUE),
            .groups = "drop")

# Create the plot
ggplot() +
  geom_jitter(data = df_num, aes(x = sample, y = fungal_cell_density),
              width = 0.2, alpha = 0.5, size = 2, color = "black") +
  
  # Smooth curve with LOESS (confidence interval included)
  geom_smooth(data = df_num, aes(x = sample, y = fungal_cell_density),
              method = "loess") +
  ...
  labs(
    title = "Fungal Cell Density vs Culture Time",
    x = "Culture Time (hours)",
    y = "Fungal Cell Density ± SD (log scale)"
  ) +
  theme_minimal() +
  theme(
